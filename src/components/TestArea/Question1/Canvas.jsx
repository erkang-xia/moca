...
const Canvas = () => {
  ...
  const doesOverlap = (newDot, existingDots) => {
    const minDistance = 60; // Minimum distance between centers of dots
    ...
    const generateDots = () => {
      ...
      while (tempDots.length < 10) {
        ...
        const newDot = {
          ...
          x: Math.floor(Math.random() * 246) + 30, // Adjust to ensure dots stay within bounds
          y: Math.floor(Math.random() * 240) + 30,
        };
  ...
  useEffect(() => {
    ...
    ctx.clearRect(0, 0, 326, 300); // Clear previous lines
    ctx.lineWidth = 3;
    ctx.strokeStyle = '#13598B';
    lines.forEach((line) => {
      ctx.beginPath();
      ...
      <div>
        <div className={styles.container}>
          ...
          <div className={styles.canvasContainer}>
            <canvas ref={canvasRef} width="326" height="300" className={styles.canvas} />
            {dots.map((dot) => (
                <button
